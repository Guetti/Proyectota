      ...
      <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 md:grid-cols-2">
        {messages.map((message, index) => {
          ...
          return (
            <Card
              key={message.id}
              className={`group relative overflow-hidden border-2 rounded-none border-border bg-card p-6 transition-all hover:border-accent hover:shadow-lg ${isLast && isOdd ? "md:col-span-2 md:max-w-2xl md:mx-auto" : ""
                }`}
            >
              ...
              <blockquote className="relative text-card-foreground text-pretty leading-relaxed">
                ...
                <p className="relative italic whitespace-pre-line">{message.message}</p>
